...
\code{Classical.choice} / \code{Quot.sound}, the named assumed-spec axioms
(characterising Aeneas-Std primitives and, in disclosed cases, this crate's own
opaquely-extracted code) and \code{bv\_decide}'s certificate axiom---is disclosed via
...
the Lean~4 kernel, the disclosed non-standard axioms (the assumed-spec axioms ---
both Aeneas-Std characterisations and disclosed crate-specific ones --- and
\code{bv\_decide}'s certificate axiom), and the \emph{Aeneas model} of the
...
\item explicit disclosure that \textbf{140 \code{kani::assume} preconditions}
...
fixed-size symbolic buffers, under 140 disclosed \code{kani::assume}
...
axiom-free: the L4 proofs rest on named assumed-spec axioms (Aeneas-Std
characterisations and, where disclosed, crate-specific ones) and
