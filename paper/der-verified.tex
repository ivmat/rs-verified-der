...
floor of 161 proof harnesses across 25 modules---each harness proving its own
asserted property (safety, and per codec some of round-trip, minimality, or
rejection) over a stated bound---and an optional, additive set of three
Aeneas${\to}$Lean~4 ``lids'' that lift three codecs (where an $\forall$-length
guarantee is most valuable and the functional model is cleanest) to inputs of
\emph{any} length, gated by a drift-detecting re-extraction check.
...
signature, or trust semantics. Out of scope (and explicitly \emph{not}
proven): cryptography, path/trust validation, and the full RFC~5280 profile.
...
\node[box, below left=9mm and -6mm of src] (l3)
  {\textbf{L3 floor} --- stable toolchain\\Kani $\to$ CBMC\\161 bounded harnesses};
...
\node[box, below=9mm of shim] (l4)
  {\textbf{L4 lids} --- Charon $\to$ Aeneas $\to$ Lean~4\\3 codecs, $\forall$-length, \code{sorry}-free};
\draw[->] (src) -- (l3);
\draw[->] (src) -- (shim);
...
161 Kani proof harnesses across 25 modules. Kani compiles each
...
bound? For three codecs---\code{length} (\S8.1.3), \code{big\_integer}
(\S8.3), and \code{oid} (\S8.19)---the Rust is additionally translated,
...
length, \code{sorry}-free (Table~\ref{tab:l4}). The L4 layer lifts exactly the
codecs where an $\forall$-length guarantee is most valuable and the functional
model is cleanest.
...
\midrule
\code{length} & every branch of \code{decode\_length}; round-trip canonicality \\
\code{big\_integer} & minimality biconditional (validate side); encode round-trip \\
\code{oid} & OID canonical-form biconditional (validate side) \\
\bottomrule
\end{tabular}
\caption{The three L4 Aeneas${\to}$Lean~4 lids. All \code{sorry}-free; the full
...
\code{lean/extract-oid/}), each of which (1)~\code{\#[path]}-includes the
...
composition glue. The crate uses four such stubs across three harnesses,
forming a DAG (\code{x509\_certificate} $\to$ \code{x509\_tbs\_certificate}
$\to$ \{\code{x509\_name} $\to$ its RDN lemma, \code{x509\_extension}\});
each link is a real function separately proven panic-free.
...
heavily right-skewed: the large majority of the 161 harnesses finish
...
CI runs the memory-tractable share of the floor (135 of 161 harnesses,
sharded by module across three parallel runners) on every push; the 26
heaviest (\code{set\_of}, \code{sequence}, \code{x509\_extension},
\code{x509\_certificate}, \code{x509\_tbs\_certificate}, \code{x509\_name})
peak above a standard 7~GB hosted runner and are documented as local-milestone
checks. On a 16-core / 29~GB Linux reference box the three CI
shards run in ${\sim}4$--$5$~min wall (\code{codecs-a} 84 harnesses ${\sim}28$~s,
\code{codecs-b} 42 ${\sim}40$~s, \code{utf8} 9 ${\sim}247$~s), and the full
suite verifies end to end with zero failures.
...
\item explicit disclosure that \textbf{130 \code{kani::assume} preconditions}
...
\item a dedicated \textbf{``what is NOT proven'' scope fence} (no cryptography,
  no full profile semantics, bounded except the three L4 codecs, the
...
rather than assumed safe---the crate's five \code{oid} Kani harnesses and its 294
...
fixed-size symbolic buffers, under 130 disclosed \code{kani::assume}
preconditions, up to stated unwind depths; inputs outside those domains are not
claimed. Only three codecs have $\forall$-length L4 lids. The correctness of the
...
From a fresh clone, \code{./check.sh} reproduces the L3 floor (161 Kani
harnesses) and the 294 tests; it additionally re-extracts and checks the three
