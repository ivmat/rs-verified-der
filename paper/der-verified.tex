...
own trust surface: both had originally treated \code{decode\_tag} as an
assumed Aeneas-Std spec (an axiom, not a body) because of the same
depth-two-loop \code{return} shape \S\ref{sec:gotcha} describes for
\code{oid}; refactoring \code{tag} the same way and proving its own lid
discharged those four \code{tag\_decode\_*} axiom instances, shrinking each of
...
\midrule
\code{length} & every branch of \code{decode\_length}; round-trip canonicality (\code{decode\_accepts\_only\_canonical}) \\
\code{big\_integer} & minimality biconditional, validate side (\code{validate\_iff\_minimal}); encode round-trip (\code{encode\_minimal\_integer\_into\_roundtrip}) \\
\code{oid} & OID canonical-form biconditional, validate side (\code{validate\_iff\_canonical}) \\
\code{tag} & \code{decode\_tag}'s totality and consumption bound (\code{tag\_decode\_total}, \code{tag\_decode\_used\_bounds}) \\
\code{tlv} & \code{decode\_tlv}'s structural correctness and no-over-read (\code{decode\_tlv\_structure}) \\
\code{sequence} & \code{decode\_sequence}'s structural correctness, $\forall$-length \emph{and} $\forall$-children (\code{decode\_sequence\_structure}) \\
\bottomrule
\end{tabular}
...
\emph{tripwire} closes the window between milestone runs: a committed
content-hash state file over the six lidded source files, re-checked on every
commit, fails closed if any of them changed without a corresponding full Lean
re-verification (or is explicitly acknowledged as pending debt)---so a lid
cannot silently go stale for several commits before the next full gate
catches it.
...
CI runs the memory-tractable share of the floor (about 136 of the 171
harnesses, filtered by module across parallel runners) on every push; the
heaviest modules (\code{set\_of}, \code{sequence}, \code{x509\_extension},
...
checks. The most recently committed full-suite run (16-core / 29~GB Linux
reference box, sequential---no \code{-j}, since parallel harnesses multiply
peak RSS---inside a 22~GB memory cgroup) verified all 171 harnesses
\code{SUCCESSFUL}, 0 \code{FAILED}, in ${\sim}91$~min wall, alongside a green
\code{cargo test} and a green L4 Lean pass in the same run.
...
codec, and the rejection/canonicality classification of even the six lidded
codecs, remains Kani-bounded. The correctness of the
