...
#   commit-msg -> hooks/commit-msg   (content-leak scan of the commit MESSAGE itself — the one
#                                     surface no tree/push gate reads)
...
chmod +x hooks/pre-commit hooks/commit-msg check_fast.sh gates/pre-push
...
echo "installed: .git/hooks/commit-msg -> hooks/commit-msg  (content-leak scan of the message)"
